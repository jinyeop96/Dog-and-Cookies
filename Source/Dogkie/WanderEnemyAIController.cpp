...
#include "WanderEnemyAIController.h"

void AWanderEnemyAIController::OnPossess(APawn* InPawn)
{
	PossessedEnemy = Cast<AEnemy>(InPawn);
	UE_LOG(LogTemp, Warning, TEXT("AWanderEnemyAIController possesses an enemy object at %s"), *PossessedEnemy->GetActorLocation().ToString());

}

void AWanderEnemyAIController::OnUnPossess()
{
	Super::OnUnPossess();
...
void AWanderEnemyAIController::Tick(float DeltaSeconds)
{
